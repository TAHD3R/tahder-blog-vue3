<template>
  <n-card embedded content-style="margin-top:24px;">
    ...
    <n-space>
      <img :src="store.userInfo['avatar']" alt="" class="w-16 h-16 sm:w-24 sm:h-24 rounded-xl" />
      <n-space vertical>
        <n-space>
          <n-space justify="end">
  ...
  <n-card class="my-4" title="我的文章">
    ...
    <n-list show-divider hoverable>
      <n-empty v-if="toLength(Object.keys(store.myArticles).length) == 0" size="huge" class="my-16" description="暂时还没有文章发表呢.." />
      <div v-else>
        <div v-for="(item, index) in store.myArticles" :key="index">
  ...
  <n-modal v-model:show="active" preset="card" title="编辑个人资料" style="width: 300px">
    <n-form ref="formRef" :model="formValue" label-placement="left" label-align="left">
      ...
      <n-form-item path="description">
        <n-input v-model:value="formValue.description" placeholder="输入自我介绍" />
      </n-form-item>
    </n-form>
    <template #footer>
      <n-space justify="center">
        <n-button strong class="w-full" @click="handleClick"> 提交修改 </n-button>
      </n-space>
    </template>
  </n-modal>
</template>
...
import { AnalyticsOutline as Fans, BalloonOutline as Follow, FlameOutline as Fire, PencilOutline as Pen } from '@vicons/ionicons5';
...
onMounted(() => {
  get_my_articles().then((res) => {
    nextTick(() => {
      console.log(res);
      store.myArticles = res.data.data;
    });
  });
});
