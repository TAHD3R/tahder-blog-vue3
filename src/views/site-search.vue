<template>
  <n-card>
    <n-list show-divider hoverable class="my-4">
      <n-empty v-if="toLength(Object.keys(store.searchResults).length) == 0" size="huge" class="my-16" description="没有找到搜索内容.." />
      <div v-else>
        <div class="text-3xl font-bold my-4">搜索内容</div>
        <div v-for="(item, index) in store.searchResults" :key="index">
