<template>
  <n-card>
    <n-alert title="滑动标签页查看更多分类" type="success" class="flex sm:hidden mb-4" />
    <n-tabs v-if="dataPrepared" animated type="line" @update:value="handleSwitch">
      <n-tab-pane v-for="(tab, key) in options" :key="key" :name="tab['id']" :tab="tab['name']">
        ...
        <n-list show-divider hoverable class="my-4">
          <n-empty v-if="toLength(Object.keys(articles_list).length) == 0" size="huge" class="my-16" description="暂时还没有文章发表呢.." />
          <div v-else>
            <div v-for="(item, index) in articles_list" :key="index">
                ...
                <div class="flex flex-row">
                  <div class="flex flex-col">
                    <div class="text-xl font-bold mt-4 sm:mt-8">
                      ...
                      <n-tag type="success" size="small" class="ml-2" :tooltip="false">
                        {{ item['category'] }}
                      </n-tag>
                    </div>
                    ...
                    <n-space class="flex justify-center items-center font-bold mb-4 sm:mb-8">
                      <div class="flex items-center">
                        <img :src="item['avatar']" class="w-8 h-8 mr-2 rounded-full" />
                        <div>{{ item['author'] }}</div>
                      </div>
                      <div class="flex items-center">
                      ...
                      <div class="hidden xl:inline">
                        <n-time :time="item['created_time']" format="yyyy年MM月dd日" unix />
                      </div>
                    </n-space>
                  </div>
                </div>
...
import { get_category_articles } from '../api/category';
